...
def get_category_client(categorie_client_id : str):
    ...
    obj: ObjectManagerInterface = ObjectManagerAdapter()
    category_client = obj.find_object_by(CustormerType, **category_client_obj)
...
def get_categories_clients():
    ...
    obj: ObjectManagerInterface = ObjectManagerAdapter()
    categories_clients = obj.find_all(CustormerType)
...
@app_views.route('/type_paiement', methods=['POST'], strict_slashes=False)
@cross_origin()
def post_type_paiement():
    """create a new category"""
    if not request.get_json():
        return make_response(jsonify(
            {'status': '401', 'message': 'The request data is empty'}), 400)
    ...
    obj: ObjectManagerInterface = ObjectManagerAdapter()
    paiement_type = obj.add_object(
        PayementType, **request.get_json())
    return make_response(jsonify(paiement_type.to_dict()), 201)